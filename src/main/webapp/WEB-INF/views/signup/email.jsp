					...
					<div class="input_Email" id="signup_space_">
						...
						<div class="col-auto">
							<button type="button" class="submit_btn_email mail_check_button" 
								id="submit_btn_forEmail" disabled="disabled">이메일 전송</button>
						</div>

					</div>
					<div>
							<div class="email_check_re1">사용 가능한 이메일 입니다.</div>
							<div class="email_check_re2">이미 사용중인 이메일 입니다.</div>
						</div>
				</div>
...
$(".mail_check_input").blur(function() {
	...
	if (inputCode == code) {								// 일치할 경우
		...
		$('.emailcheckfrom').attr("action","signupinfo");
...
$('.mail_input').on("input", function() {
	var sendbtn = $('.mail_check_button');
	var member_email = $('.mail_input').val();	
	var data = {'member_email' : member_email}
	
	if (!member_email) {
		 $('.email_check_re1').css("display", "none");
		 $('.email_check_re2').css("display", "none");
	      return;
	    }
	$.ajax({
		type : "post",
		url : "${path}/emailcheck/memberemailChk",
		data : data,
		success: function(result){
		if (result != 'fail') {
			
			$('.email_check_re1').css("display", "inline-block");
			$('.email_check_re2').css("display", "none");
			sendbtn.attr("disabled",false);
		} else {
			
			$('.email_check_re1').css("display","none");
			$('.email_check_re2').css("display", "inline-block");
